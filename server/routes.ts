...
function hasRole(...roles: string[]) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!roles.includes(user.role)) return res.status(403).json({ message: "Sin permisos" });
    next();
  };
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/emergency-reset-admin", async (req, res) => {
    try {
      ...
      if (!existing) {
        await storage.createUser({ name: "Admin", email, password: hashed, role: "admin" });
        res.json({ status: "CREATED", message: "Admin creado" });
      } else {
        await db.update(users).set({ password: hashed, role: "admin" }).where(eq(users.email, email));
        res.json({ status: "UPDATED", message: "Pass reseteada" });
      }
