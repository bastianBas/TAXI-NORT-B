...
export class DatabaseStorage implements IStorage {
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    const newUser: User = { 
      ...
      id, 
      createdAt: new Date(),
      role: insertUser.role ?? "driver" // Valor por defecto
    };
  ...
  async createDriver(insertDriver: InsertDriver): Promise<Driver> { 
    const id = randomUUID(); 
    const newDriver: Driver = { ...insertDriver, id, createdAt: new Date(), status: insertDriver.status ?? "active" }; 
    await db.insert(drivers).values(newDriver); return newDriver; 
  }
  ...
  async createVehicle(insertVehicle: InsertVehicle): Promise<Vehicle> { 
    const id = randomUUID(); 
    const newVehicle: Vehicle = { ...insertVehicle, id, createdAt: new Date(), status: insertVehicle.status ?? "active" }; 
    await db.insert(vehicles).values(newVehicle); return newVehicle; 
  }
  ...
  async createRouteSlip(insertSlip: InsertRouteSlip): Promise<RouteSlip> { 
    ...
    const newSlip: RouteSlip = { 
      ...insertSlip, id, isDuplicate, createdAt: new Date(), 
      paymentStatus: insertSlip.paymentStatus ?? "pending", notes: insertSlip.notes ?? null, signature: insertSlip.signature ?? null, totalAmount: insertSlip.totalAmount ?? 0, expenses: insertSlip.expenses ?? 0, netAmount: insertSlip.netAmount ?? 0 
    }; 
    await db.insert(routeSlips).values(newSlip); return newSlip; 
  }
  ...
  async createPayment(insertPayment: InsertPayment): Promise<Payment> { 
    ...
    const newPayment: Payment = { 
      ...insertPayment, id, createdAt: new Date(), 
      status: insertPayment.status ?? "pending", proofOfPayment: insertPayment.proofOfPayment ?? null 
    }; 
    await db.insert(payments).values(newPayment); return newPayment; 
  }
  ...
  async createAuditLog(insertLog: InsertAuditLog): Promise<AuditLog> { 
    ...
    const newLog: AuditLog = { 
      ...insertLog, id, timestamp: new Date(), 
      entityId: insertLog.entityId ?? null, details: insertLog.details ?? null 
    }; 
    await db.insert(auditLogs).values(newLog); return newLog; 
  }
